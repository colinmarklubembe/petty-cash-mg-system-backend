    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
      ...
      <div style="padding: 20px">
        ...
        <p style="text-align: center">
          <a
            href="${process.env.BASE_URL}/api/v1/auth/verify?token=${emailData.token}"
        ...
        <div style="color: #333; font-size: 12px; max-width: 550px; margin: 20px auto 0;">
          ...
          <a href="${process.env.BASE_URL}/api/v1/auth/verify?token=${emailData.token}">${process.env.BASE_URL}/api/v1/auth/verify?token=${emailData.token}</a>
          </p>
        </div>
      </div>
    </div>
